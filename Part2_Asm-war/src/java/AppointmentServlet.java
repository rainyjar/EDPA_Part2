...
public class AppointmentServlet extends HttpServlet {
    ...
    /**
     * Handle AJAX request to get available time slots for doctors on selected
     * date
     */
    ...
    /**
     * Handle AJAX request to get doctors for a specific treatment (used by
     * reschedule form)
     */
    ...
    /**
     * Get doctors available for a specific treatment (by treatment-doctor
     * relationship)
     */
    ...
    /**
     * Handle appointment booking request - supports both customer and counter
     * staff booking
     */
    private void handleBookAppointment(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!isValidAppointmentTime(appointmentTime)) {
                if (isStaffBooking) {
                    response.sendRedirect(request.getContextPath() + "/AppointmentServlet?action=book&error=invalid_time");
                } else {
                    // Fetch and attach the data again before forwarding
                    request.setAttribute("doctorList", doctorFacade.findAll());
                    request.setAttribute("treatmentList", treatmentFacade.findAll());
                    request.setAttribute("error", "invalid_time");
                    request.getRequestDispatcher("/customer/appointment.jsp").forward(request, response);
                }
                return;
            }
...
//    @Override
     ...
     * @param excludeAppointmentId Appointment ID to exclude (for rescheduling),
     * null for new bookings
     * @return AvailabilityResult containing availability status and detailed
     * reason
     */
    ...
    /**
     * Check for appointment time conflicts during reschedule (excludes the
     * appointment being rescheduled) Now uses comprehensive availability
     * checking
     */
    ...
    /**
     * Check for appointment time conflicts for new bookings Now uses
    ...
    /**
     * Simple and efficient overdue checking - just check database status No
     * complex date calculations or mass updates needed
     */
     ...
     * An appointment is overdue if its date and time have passed but status is
     * still "approved" or "reschedule" THIS METHOD HAS BEEN DISABLED because it
     * causes infinite loading due to: 1. Loading ALL appointments from database
     * (not just current user) 2. Performing individual database UPDATE
     * operations in a loop 3. Complex date/time calculations for every
     * appointment 4. Being called on every page load
     *
     * RECOMMENDATION: Move this to a scheduled background task or admin
     * function
     */
    ...
    /**
     * Automatically update appointments that are overdue This method
     * efficiently checks and updates appointment statuses in the database An
     ...
     * updateOverdueAppointments() but with additional manual triggering
     * capabilities for testing/admin use
     */
    ...
    /**
     * Get current overdue appointments count (for reporting/dashboard) This
     * method returns the count without performing updates
     */
    ...
    /**
     * Handle manage appointments page - load and filter appointments for
     * counter staff
     */
    ...
    /**
     * Handle require reschedule request - change status to "reschedule
     * required"
     */
