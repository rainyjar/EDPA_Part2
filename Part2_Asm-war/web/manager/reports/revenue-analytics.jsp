    ...
    if (appointmentFacade != null && doctorFacade != null && paymentFacade != null) {
        try {
            ...
            for (Appointment appointment : allAppointments) {
                ...
                if (!hasPayment || (!isPaid && hasPayment)) {
                    String status = appointment.getStatus();
                    if (status != null && ("Pending".equalsIgnoreCase(status) || "Scheduled".equalsIgnoreCase(status))) {
                        if (!hasPayment) pendingCount++;
                    } else {
                        if (!hasPayment) overdueCount++;
                    }
                }
            }
                ...
                data.topDoctors.forEach((doctor, index) => {
                    ...
                    tableHtml += `
                        <tr>
                            ...
                            <td>RM ${doctor.revenue.toLocaleString()}</td>
                            <td>RM ${avgPerAppointment.toFixed(2)}</td>
                        </tr>
                    `;
                });
